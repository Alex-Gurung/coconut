    ...
    with open(input_path, 'r', encoding='utf-8') as f:
        for line_num, line in enumerate(tqdm(f, total=total_lines, desc=f"Processing {input_path.name}"), 1):
            try:
                ...
                # Minimal cleaning - preserve XML tags and content
                steps = []
                for step in raw_steps:
                    cleaned_step = minimal_clean_step(step)
                    if cleaned_step and len(cleaned_step) > 5:  # Very minimal filtering
...
def main():
    print("=== Conservative Dataset Transformation ===")
    print("Preserving original reasoning structure...")
    
    # Load spacy model
    print("Loading spacy model...")
    nlp = spacy.load("en_core_web_sm")
    
    # Define input and output paths - saving to ncp_data
    ...
    if train_output.exists():
        with open(train_output, 'r') as f:
            data = json.load(f)
            if data:
                print(f"\n=== Sample Conservative Transformation ===")
                sample = data[0]
                ...
                print(f"First 3 steps (preserving XML tags):")
                for i, step in enumerate(sample['steps'][:3]):
                    print(f"  {i+1}: {step}")
                
    print(f"\n=== Usage ===")
    print(f"Files saved to ncp_data/ directory as requested")
    print(f"XML tags like <citation> and <reasoning> are preserved")
    print(f"Only minimal whitespace cleaning applied")
